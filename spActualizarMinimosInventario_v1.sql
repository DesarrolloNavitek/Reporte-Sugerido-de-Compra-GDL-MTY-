...
/******/
--SET NOCOUNT ON;
--EXEC spActualizarMinimosInventario '15',30
...
BEGIN
   DECLARE 
	...
	SET @FDemandaProm = CASE WHEN @FDemandaProm < 20 THEN 20 ELSE @FDemandaProm END
...
;WITH ExtraeDatos AS (
    SELECT
        ...
        vt.Articulo, 
        vt.Almacen,
        SUM(vt.Cantidad) AS CantidadFactura
    FROM VentaTCalc vt
    JOIN MovTipo mt ON vt.Mov = mt.Mov AND mt.Modulo = 'VTAS'
    WHERE vt.Almacen = @Almacen
      AND mt.Clave = 'VTAS.F' 
      AND vt.Mov IN ('Factura Com.Ext40', 'Factura')
    ...
    SELECT
        ...
        vt.Articulo, 
        vt.Almacen,
        SUM(CASE WHEN vt.Cantidad = vt.CantidadPendiente 
                 THEN vt.Cantidad 
                 ELSE ISNULL(vt.CantidadPendiente, 0) END) AS CantidadFactura
    FROM VentaTCalc vt
    JOIN MovTipo mt ON vt.Mov = mt.Mov AND mt.Modulo = 'VTAS'
    WHERE vt.Almacen = @Almacen
      AND mt.Clave = 'VTAS.P' 
      AND mt.SubClave = 'VTAS.PNVK' 
      AND vt.Mov <> 'Cotizacion' 
      AND vt.Estatus = 'PENDIENTE' 
      AND vt.Situacion = 'Autorización de Pedido' 
      AND vt.Mov NOT LIKE 'PS%'
      AND vt.FechaEmision BETWEEN @FechaD AND @FechaA
    GROUP BY vt.Articulo, vt.Almacen
), UniversoArticulos AS (

SELECT 
    Almacen,
    Articulo,
    SUM(CASE WHEN Mov = 'Factura' THEN CantidadFactura ELSE 0 END) AS Demanda90,
    SUM(CASE WHEN Mov = 'Pedido'  THEN CantidadFactura ELSE 0 END) AS Pedido
FROM ExtraeDatos
...
SELECT 
    ...
    b.Descripcion1,
    b.Unidad,
    ROUND(COALESCE(a.Demanda90, 0),4)	AS DEMANDA90,
    ROUND(COALESCE(a.Pedido, 0),4)		AS PEDIDO,
    c.Factor,
    ISNULL(d.Disponible, 0) AS Disponible
FROM UniversoArticulos  a
JOIN Art                b   ON a.Articulo=b.Articulo
JOIN ArtUnidad          c   ON a.Articulo = c.Articulo
LEFT JOIN ArtDisponible d   ON a.Articulo=d.Articulo AND a.Almacen = d.Almacen
WHERE b.Articulo NOT LIKE 'ESP%'
  --AND b.Linea = '1-PT'
  --AND a.Almacen = @Almacen --IN ('10','15')
...
--SugRedon: Dividir el sugerido entre el contenido de la caja, esto da las cajas a pedir, aplicar CEILING para redondear hacia arriba el total de cajas
...
SELECT b.Almacen,
		...
		CASE WHEN Sugerido < 0.00 THEN 0 ELSE (FLOOR((Sugerido / Factor)) * Factor) END AS SugRedon
  FROM Sugerido 		a
  JOIN Totales		b ON a.Articulo=b.Articulo AND a.Almacen=b.Almacen
)
...
SELECT a.Almacen,
  ...
  GROUP BY a.Almacen,		
		...
		SugRedon
ORDER BY 5 DESC
--EXEC spActualizarMinimosInventario '15',20
END
...
BEGIN
   DECLARE 
    ...
    ;WITH UniversoArticulos AS (SELECT
        ...
        SUM(CASE WHEN mt.Clave = 'VTAS.P' 
                  AND mt.SubClave = 'VTAS.PNVK' 
                  AND vt.Mov <> 'Cotizacion' 
                  AND vt.Estatus = 'PENDIENTE' 
                  AND Situacion = 'Autorización de Pedido' 
                  AND mt.Mov NOT LIKE 'PS%' THEN 
            CASE WHEN vt.Cantidad = VT.CantidadPendiente THEN vt.Cantidad ELSE vt.CantidadPendiente END ELSE 0 END) AS Pedido
	FROM VentaTCalc vt
...
SELECT a.Almacen,
  ...
  GROUP BY a.Almacen,		
		...
		SugRedon
ORDER BY 5 DESC
--EXEC spActualizarMinimosInventario '10',20
END
*/
